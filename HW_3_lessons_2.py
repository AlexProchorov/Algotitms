...
def fucntion(number, reverse_list=[]):
    digital_number = number % 10
    new_number = number // 10
    reverse_list.append(digital_number)

    if new_number == 0:
        result = ''
        for number in reverse_list:
             result += str(number)
        print(f'Результирующее число равно {result}')
        return
    fucntion(new_number, reverse_list)
